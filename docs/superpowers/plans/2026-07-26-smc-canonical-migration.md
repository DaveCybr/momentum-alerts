...
**Type consistency:** `SmcState(symbol,state,data)` and `.to_json()/.from_json()` used identically in Tasks 1/8/9/15. `evaluate(bundle,params,state) -> (Setup|None, SmcState)` consistent across Task 8/15. `place_limit(...) -> dict{ok,lot,entry,order_ticket,est_risk,spread,risk_pct}` consumed in Task 12. Filled position tickets are reconciled separately from pending order tickets. `daily_counts(day_prefix) -> {losses,trades,net_r}` produced Task 9, consumed Task 11. `candle_sent(symbol,candle_id,direction)` new arity used in Task 9/16.

**Pre-flight corrections (spec governs):** protected-level invalidation only scans body closes after the BOS that established the active bias; POI origin indices persisted in state are absolute timestamps/indices rather than sliced-window positions; demo-account detection fails closed when `account_info()` or `trade_mode` is missing; pending lifecycle enforces all §11 cancellation rules (3 M5 closes, target-first, sweep body-close violation, POI distal invalidation, session end, RR degradation; news remains tagged/deferred); broker order tickets and filled position tickets are stored separately so reconciliation does not assume they are identical.
